...
async def scrape_studies():
    base_url = "https://pbs.edu.pl/pl/oferta-dydaktyczna/studia-podyplomowe"
    async with AsyncWebCrawler(verbose=True) as crawler:
        ...
        try:
            ...
            if result.success:
                ...
                # Save to file
                with open("pbs_studies_postgraduate.json", "w", encoding="utf-8") as f:
                    json.dump(parsed_data, f, ensure_ascii=False, indent=2)

                if parsed_data.get('programs'):
                    print(f"SUCCESS! Saved {len(parsed_data['programs'])} programs to pbs_studies_postgraduate.json")
                else:
